...
#Define     amplify_factor_m       FPar_17  ' define amplification facor for safe parameter m
...
#define     vibration_amplitude_modelcontrol   FPAR_23  'Wird von Labview beschrieben
rem define penetration velocity
#Define     penetration_vel              FPAR_24     'Wird von Labview beschrieben
...
#Define     PreShearingOutput            FPAR_28     'Wird von Labview beschrieben
#Define     start_penetrat_depth         FPAR_29     'Wird von Labview beschrieben
#Define     initial_strain               FPAR_30     'Wird von Labview beschrieben
#DEFINE     initial_cone_position        Fpar_31     'Wird von Labview beschrieben
#define     volume_change                FPAR_32     'Wird von Labview beschrieben
#define     actual_volume_strain         FPAR_33     'Wird von Labview beschrieben
#Define     estimated_slope              FPAR_34     'Wird von Labview beschrieben
#Define     estimated_intercept          FPAR_35     'Wird von Labview beschrieben
#define     Soll_Position_Regelungswert  FPAR_36     'Wird von Labview beschrieben
#Define     Vibro_mode_2_amplitude  FPAR_37          'Wird von Labview beschrieben
#Define     Vibro_mode_2_frequency  FPAR_38          'Wird von Labview beschrieben
#Define     Vibro_mode_2_background_vel  FPAR_39     'Wird von Labview beschrieben
...
#define    Switch_freq_depth_static FPAR_46    'Wird von Labview beschrieben
#define    time_delay_change_freq_2  Fpar_47   'Wird von Labview beschrieben
#define    Vibro_mode_3_amplitude  FPAR_48     'Wird von Labview beschrieben
#define    Vibro_mode_3_frequency  FPAR_49     'Wird von Labview beschrieben
#define    Volume_triangular       FPAR_50     'Wird von Labview beschrieben 
...
#Define     Vibro_mode_3_background_vel  FPAR_64  'Wird von Labview beschrieben
#define     time_delay_change_freq  Fpar_65       'Wird von Labview beschrieben
#define     time_delay_change_freq_1  Fpar_66     'Wird von Labview beschrieben
#Define     Model_PreShearing      FPAR_67        'Wird von Labview beschrieben
#Define     Model_Vibro            FPAR_68        'Wird von Labview beschrieben
#Define     Switch_freq_depth      Fpar_69        'Wird von Labview beschrieben
#Define     Switch_freq_depth_1    Fpar_70        'Wird von Labview beschrieben
#Define     stopbutton             Fpar_71        'Wird von Labview beschrieben
...
#Define     Taktzeit               FPAR_78     'Wird von Labview beschrieben 
#Define     Abtastfrequenz         FPAR_79     'Wird von Labview beschrieben
#Define     Clock                  FPAR_80     'Wird von Labview beschrieben 

#define     fifo_transmitflag      Par_12       'Wird von Labview beschrieben
...
#Define     switch_frequency_button    PAR_6    'Wird von Labview beschrieben
#define     program_counter_interval   Par_7    'Wird von Labview beschrieben
...
#define virboswitch_flag   par_10               'Wird von Labview beschrieben 
rem to switch for third vibro mode
#define virboswitch_flag_1   par_11             'Wird von Labview beschrieben

rem define bug flag to indicate appearance of spike bug
rem modified by zhengyu chen 2022-05-10         
#define     BUG_flag               PAR_12        'Wird von Labview beschrieben

#Define     Amplitude_PreSh        PAR_13       'Wird von Labview beschrieben 
#Define     Compass_OFF            PAR_14       'Wird von Labview beschrieben
#define     test_finish_flag       Par_15       'Wird von Labview beschrieben

#define    control_cycle              PAR_16     'Wird von Labview beschrieben

#define teste_cycle par_18                       'Wird von Labview beschrieben

#define  start_pump_test par_19                 'Wird von Labview beschrieben
...
#Define     Enable_vibro_mode_2    PAR_34       'Wird von Labview beschrieben
#Define     Enable_vibro_mode_3    PAR_35       'Wird von Labview beschrieben
#Define     Final_vibromode_switch PAR_36       'Wird von Labview beschrieben
#Define     Circumference_ON_Flag  PAR_37       'Wird von Labview beschrieben 
#Define     controlstart_step      Par_39       'Wird von Labview beschrieben
...
#Define      total_shift           par_45       'Wird von Labview beschrieben
#Define     Einfahren_Piston       PAR_46       'Wird von Labview beschrieben
#define     Switch_freq_condition_fulfil FPAR_49  'Wird von Labview beschrieben
#Define     total_shift_max        PAR_53         'Wird von Labview beschrieben
rem define global pointer for drift movement of cone#
#Define     global_pointer         PAR_54         'Wird von Labview beschrieben
...
#Define     LCT_strain_based_control_activate PAR_66  'Wird von Labview beschrieben
#Define     LCT_strain_based_trigger_condition  PAR_67   'Wird von Labview beschrieben
#Define     Pre_Shearing_Button    PAR_70       'Wird von Labview beschrieben
#Define     linear_calibration_flag  par_72     'Wird von Labview beschrieben
...
rem define current background velocity
DIM current_p_vel as float 
#define final_switch_flag par_20
...
DIM desired_distance_phy_transmit as float
...
rem define index
DIM initial_index as long
DIM initial_muanuel_move_index as long
...
rem declaration for side and pore volume 
DIM initial_side_volume as float    
DIM side_volume_pointer as long
DIM pore_volume_pointer as long
DIM initial_pore_volume as float

rem declaration for sample parameters
DIM sample_height as float
dim sample_radius as float
DIM sample_volume as float 
rem ####################################################################
rem ####################################################################
rem vairable declaration kalman filter
DIM K_1[2] as float
DIM K_2[2] as float
DIM measured_press_rate as float
DIM measured_pressPore as float

DIM measured_pressSide as float

DIM measured_pressAxial as float

DIM pressure_rate as float
DIM dt as float
DIM r_t[2] as float
rem for pore pressure
DIM Porepressure_rate_prior as float
DIM Porepressure_rate_estimate as float
DIM Porepress_digits_prior as float
DIM Porepress_digits_estimate as float
DIM Porepredict_error_cov_prior as float
DIM Porepredict_error_cov as float
DIM Porepredict_error_cov_vel_prior as float
DIM Porepredict_error_cov_vel as float

rem for pore pressure
DIM Sidepressure_rate_prior as float
DIM Sidepressure_rate_estimate as float
DIM Sidepress_digits_prior as float
DIM Sidepress_digits_estimate as float
DIM Sidepredict_error_cov_prior as float
DIM Sidepredict_error_cov as float
DIM Sidepredict_error_cov_vel_prior as float
DIM Sidepredict_error_cov_vel as float

rem for pore pressure
DIM Axialpressure_rate_prior as float
DIM Axialpressure_rate_estimate as float
DIM Axialpress_digits_prior as float
DIM Axialpress_digits_estimate as float
DIM Axialpredict_error_cov_prior as float
DIM Axialpredict_error_cov as float
DIM Axialpredict_error_cov_vel_prior as float
DIM Axialpredict_error_cov_vel as float


rem array to record 2 consecutive measured pressure
DIM measure_press_array[2] as float
rem array pointer
DIM press_p as long
...
DIM axial_prss_array[arraysize] as float
Dim side_prss_array[arraysize] as float
DIM pore_prss_array[arraysize] as float

DIM pore_press_pointer as long
DIM side_press_pointer as long
DIM axial_press_pointer as long

rem linear model paras for disturbance due to vibro CPT
#define pore_m 0.0564360698
#define pore_b 0.0125244
#define side_m 0.055910282595
#define side_b 0.012420103696
#define axial_m 0.07277861
#define axial_b 0.015435599

#define time_delay_pore 100
#define time_delay_side 200
#define time_delay_axial 160

DIM estimated_pore_press as float
DIM estimated_side_press as float
DIM estimated_axial_press as float

#define press_intervalp 200

DiM newSet_Soll_PorePressure AS float
DiM newSet_Soll_SidePressure AS float
DiM newSet_Soll_AxialPressure AS float
...
rem date: 2022.04.29
rem   data_1 : 30000, original size:5000
rem   data_12: 30000, original size: 5000
rem   data_50: 260000, original size: 80000
...
DIM data_50[260000] as Float as Fifo  'main FIFO output 

DIM data_2[10000] as Float as Fifo
...
rem model parameters for LVDT response linear model
DIM Advance_cone_response_slope as float
DIM Advance_cone_response_intercept as float
DIM advance_depth_sample as float
DIM r1 as float
DIM r2 as float
DIM h_p as float
rem define  initial side pressure for vcpt test
DIM initial_sidepress_digits as float
...
Init:
  ...
  accleration_full = 0
  initial_index = 0
  initial_muanuel_move_index = 0
  
  Final_vibromode_switch = 0
  ...
  rem initialize p
  p = 1
  LCT_strain_based_control_activate = 0
  pore_volume_pointer = 1
  actual_volume_strain = 0
  '------------------------------------------------------------------------------------------------------------------------------------------ 
  '------------------------------------------------------------------------------------------------------------------------------------------ 
  ...
  '------------------------------------------------------------------------------------------------------------------------------------------ 
  '------------------------------------------------------------------------------------------------------------------------------------------ 
  rem initialization of Kalman filter
 
  dt = 1
  rem calculate noise
  rem process noise variance
  r_t[1] =  2.5   
  rem observation noise variance
  r_t[2] = 0.12
  press_p = 1 
  rem for pore pressure
  Porepress_digits_estimate = 0
  Porepressure_rate_estimate = 0
  Porepredict_error_cov_prior= 0
  Porepress_digits_prior = 0
  rem predict error covariance
  Porepredict_error_cov = 300
  Porepredict_error_cov_vel = r_t[2]
  
  rem for side pressure
  Sidepress_digits_estimate = 0
  Sidepressure_rate_estimate = 0
  Sidepredict_error_cov_prior= 0
  Sidepress_digits_prior = 0
  rem predict error covariance
  Sidepredict_error_cov = 300
  Sidepredict_error_cov_vel = r_t[2]
  
  rem for pore pressure
  Axialpress_digits_estimate = 0
  Axialpressure_rate_estimate = 0
  Axialpredict_error_cov_prior= 0
  Axialpress_digits_prior = 0
  rem predict error covariance
  Axialpredict_error_cov = 300
  Axialpredict_error_cov_vel = r_t[2]
  '------------------------------------------------------------------------------------------------------------------------------------------ 
  '------------------------------------------------------------------------------------------------------------------------------------------ 
  ...
  '  RS_INIT(1,460800,0,8,0,3)
 
  RS_INIT(1,230400,0,8,0,3)
  RS485_SEND(1,0)
  '  RS_INIT(0,230400,0,8,0,3)
  '  RS485_SEND(0,0)
  FIfO_clear(1)
  ...
  rem initialize pressure array pointer
  ...
  rem initialization of NEW bc5
  
  r1 = 6.553   
  ' unit mm
  Advance_cone_response_slope = -0.2228
  Advance_cone_response_intercept = 140.75
...
Event:
  ...
  rem initialize amplification factor for safe para m
  
  amplify_factor_m = 1.2    +0.15*((vibration_amplitude_modelcontrol/2) -1)
     
  if (virboswitch_flag = 1 ) then amplify_factor_m = 1.2    +0.15*((Vibro_mode_2_amplitude/2) -1)
  if (virboswitch_flag_1  = 1) then amplify_factor_m = 1.2    +0.15*((Vibro_mode_2_amplitude/2) -1) 
  '  amplify_factor_m =1
  ...
  rem use kalman filter to estimate pressure
  
  
  if (press_p <= 2) then
    measure_press_array[press_p] = ((12000 / 65536) * (PorePress_Digits)) - 6000
    inc(press_p)
  else
          
    measure_press_array[1] =  measure_press_array[2]
    measure_press_array[2] = ((12000 / 65536) * (PorePress_Digits)) - 6000
    inc(press_p)
  endif

  rem calculate measured pressure rate
  measured_press_rate =  measure_press_array[2]- measure_press_array[1]
  rem measured pressure in Kpa    
  measured_pressPore = ((12000 / 65536) * (PorePress_Digits)) - 6000
  measured_pressSide = ((12000 / 65536) * (SidePress_Digits)) - 6000
  measured_pressAxial = ((12000 / 65536) * (AxialPress_Digits)) - 6000
      
  rem  kalman filter for estimation of pore, side and axial pressure
  Kalman_filter(K_1,K_2,measured_press_rate,measured_pressPore,dt,r_t,Porepressure_rate_prior,Porepressure_rate_estimate,Porepress_digits_prior,Porepress_digits_estimate,Porepredict_error_cov_prior, Porepredict_error_cov,  Porepredict_error_cov_vel_prior,Porepredict_error_cov_vel)
     
  Kalman_filter(K_1,K_2,measured_press_rate,measured_pressSide,dt,r_t,Sidepressure_rate_prior,Sidepressure_rate_estimate,Sidepress_digits_prior,Sidepress_digits_estimate,Sidepredict_error_cov_prior, Sidepredict_error_cov,  Sidepredict_error_cov_vel_prior,Sidepredict_error_cov_vel)
      
  Kalman_filter(K_1,K_2,measured_press_rate,measured_pressAxial,dt,r_t,Axialpressure_rate_prior,Axialpressure_rate_estimate,Axialpress_digits_prior,Axialpress_digits_estimate,Axialpredict_error_cov_prior, Axialpredict_error_cov, Axialpredict_error_cov_vel_prior,Axialpredict_error_cov_vel)
     
 
  rem use signal after kalman filter as input of PI controller
  PorePress_Digits = (Porepress_digits_estimate + 6000)/((12000 / 65536) )
  SidePress_Digits = (Sidepress_digits_estimate + 6000)/((12000 / 65536) )
  AxialPress_Digits = (Axialpress_digits_estimate + 6000)/((12000 / 65536) )
  
  
  press_array_append(axial_prss_array,side_prss_array, pore_prss_array,pore_press_pointer,side_press_pointer,axial_press_pointer, arraysize, PorePress_Digits, SidePress_Digits,AxialPress_Digits)
  ...
  Selectcase Modus
    ...
    Case Model_based_control
      ...
      if ((controlstart_step = 0)   )  then
        ...
        desired_distance_phy_transmit = WegInd_Digits
      ...
      if ((controlstart_step = 1) )  then
        ...
        '        if (final_switch = 0) then
        '          linear_coff[1] =    -0.000301003  *amplify_factor_m
        '          linear_coff[2] =     12.9918  
        '          
        '          
        '        endif          
        rem modify the estimated model parameters if thez deviates away from safe ones too much
        '        if (switch_flag = 1) then
        if (final_switch = 1) then
        
          estimated_parameters_justification(amplify_factor_m,Start_Test_Button,full_flag,estimated_parameter_pointer,linear_coff,estimated_slope_array,estimated_intercept_array)  
    
        else
          rem hold the position of piston when final switch of model control is not engaged  
          if (control_cycle = 0) then
            Soll_Position_Regelungswert = WegInd_Digits
            inc(control_cycle)
      ...
      rem calculate predict distance
      ...
      ' Pressure control with PI controller for pore, axial and side pressure regulation  
      
      
     
      '      if (final_switch = 1) then
      pressure_disturbance_estimation(velocity_data,local_vel_pointer, estimated_pore_press, estimated_side_press, estimated_axial_press,arraysize,axial_prss_array,side_prss_array, pore_prss_array,pore_press_pointer,side_press_pointer,axial_press_pointer,press_intervalp,data_sizetrain, pore_m,side_m,axial_m,pore_b,side_b,axial_b, time_delay_pore, time_delay_side, time_delay_axial)

      '----------------------------------------------------------------------------------------------------------------
      rem incorporate disturbance term into pressurement measuement
     
      if (final_switch = 1) then  
        newSet_Soll_PorePressure = ((press_intervalp*(pore_m*vel_local  + pore_b))/(12000 / 65536)  + Set_Soll_PorePressure )  
        newSet_Soll_sidePressure = ((press_intervalp*(side_m*vel_local  + side_b))/(12000 / 65536)  + Set_Soll_PorePressure ) 
        newSet_Soll_axialPressure = ((press_intervalp*(axial_m*vel_local  + axial_b))/(12000 / 65536)  + Set_Soll_PorePressure ) 
        '  
     
      else
        newSet_Soll_PorePressure =  Set_Soll_PorePressure
        newSet_Soll_sidePressure = Set_Soll_SidePressure
        newSet_Soll_axialPressure = Set_Soll_AxialPressure
      endif  
    ...
    Case MODUS_MAIN_CONTROL 'all Vibro/Spindel/Pressure Control 
      ...
      '-----------------------------------------------------------------------------------------------------------------------------------------------------------
      '------------------------------------------------------------------------------------------------------------------------------------------
      '  LCT strain based control           
      if (LCT_strain_based_control_activate = 1) then
       
        rem #####################################################################
        rem mode trigger condition is based on side volume change
        rem detect change in side volume 
        if (LCT_strain_based_trigger_condition = 1) then
          ...
          actual_volume_strain = abs(0.03107*(SpindelB_Digits-initial_side_volume))
          '        /((0.03107* initial_side_volume) - 1015.90 )
          rem if side volume change exceeds set value, stop loading
          if (actual_volume_strain >= volume_change) then 
        ...
        rem #######################################################################
        rem  trigger condition is based on volumetric strain from pore volume change 
        if (LCT_strain_based_trigger_condition = 2) then
                
          rem calculate volumetric strain based on pore volume change
                  
          if (pore_volume_pointer = 1) then
            rem record initial digital side volume 
            initial_pore_volume = SpindelA_Digits
          endif
          inc(pore_volume_pointer)
          rem actual pore volume change in mm^3
          rem define sample height in mm, radius in mm
          sample_height = 545
          sample_radius = 150
          rem define sample volume
                  
          sample_volume = pi*sample_height*sample_radius*sample_radius
          actual_volume_strain = (1000*abs(0.03107*(SpindelA_Digits-initial_pore_volume)))/sample_volume
          '        /((0.03107* initial_side_volume) - 1015.90 )
          rem if side volume change exceeds set value, stop loading
          if (actual_volume_strain >= volume_change) then 
      ...
      '      if (teste_cycle = 1) then 
      '        initial_control = AxialPress_Digits 
      '      endif
      '      
      '      rem ############################################################
      '      if (start_pump_test = 1) then
      '        
      '        
      '        if ( ((12000/65536)*(AxialPress_Digits)-6000) < 45) then
      '          AxialPress_Digits  = initial_control + 0.002*(teste_cycle)
      '        else  
      '          AxialPress_Digits  = initial_control - 0.002*(teste_cycle)  
      '          
      '        endif
  ...
  If (synced_ >=2) Then
    If (fifoused_ > 38) Then
      ...
      For z = 5 to 8
        ...
        '        dummy = shift_left(dummy, 8)
        '        dummy = dummy + FIFO_RS_SYNCED[4*z+1]  
  ...
  if ( program_counter_interval = 1 )THEN
    if (BUG_flag <= 1) then
      if (FIFO_Empty(50) > 30)  Then
        ...
        FIFO_MAIN_OUT = desired_distance_phy_transmit                                  ' 20 desired distance in mm
        ...
        FIFO_MAIN_OUT = h_p                                                             ' 38 advanced depth in Linear model for BC5 WITH LVDT response
        FIFO_MAIN_OUT = r2                                                              ' 39 outer radius for linear model
